...
# Rewritten ackermann class to always control acceleration
...
class CarlaAckermannControl(object):
    ...
    def __init__(self):
        ...
        self.clock_frequency = rospy.get_param("clock_frequency", default=10)

        self.control_loop_rate = rospy.Rate(self.clock_frequency)  # 10Hz
        ...
        sys.modules['simple_pid.PID']._current_time = (       # pylint: disable=protected-access
            lambda: rospy.get_rostime().to_sec() - (1.0 / self.clock_frequency))

        self.accel_controller = PID(Kp=0.0,
                                    Ki=0.0,
                                    Kd=0.0,
                                    sample_time=(0.5 / self.clock_frequency),
                                    output_limits=(-1, 1))
    ...
    def vehicle_control_cycle(self):
        ...
        # Compute the actual throttle value to reach the desired acceleration
    ...
    def control_steering(self):
        """
        Basic steering control (No dynamics here!)
        """
        # The restriction is not updated, while there is a steering curve...
        # Steering curve tested:
        # Results: (0, 1), (20, 0.899), (60, 0.8), (120, 0.699).
        apply_correction = lambda v: (1.0 - (1.0 - 0.899)/20.0 * v)  # Valid up until 20 m/s

        self.info.output.steer = self.info.target.steering_angle / \
                                 (apply_correction(self.vehicle_status.velocity)*self.info.restrictions.max_steering_angle)
    ...
    def run_accel_control_loop(self):
        ...
        # print("target {}, delta {}, output {}".format(self.accel_controller.setpoint, \
        #                                               self.info.status.accel_control_pedal_delta, \
        #                                               self.info.status.accel_control_pedal_target))
    ...
    def update_drive_vehicle_control_command(self):
        ...
        if self.info.status.accel_control_pedal_target > self.info.status.throttle_lower_border:
            ...
            # print(abs(self.info.restrictions.max_pedal))
            # print('Accelerating')
        ...
        # We are getting messages of unexpected control inputs, but it seems to be due to the frequent timing
        # rather than the output values
        # print("[Output Ackermann]\nthrottle: {}\nbrake: {}\nsteering: {}".format(self.info.output.throttle,
        #                                                                         self.info.output.brake,
    ...
    def update_current_values(self):
        ...
        if delta_time > 0:
            ...
            # average filter!!
            self.info.current.accel = (self.info.current.accel * 2 + current_accel) / 3 # was 4 / 5
        ...
        # print("Current acceleration: {}".format(self.info.current.accel))
